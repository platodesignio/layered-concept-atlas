...
const layers = [
  {
    ...
    slug: "l0",
    nameJa: "L0 生成位相",
    descriptionJa: "存在の根源的な発生・出現・潜在性の位相",
    colorClass: "layer-0",
  },
  {
    index: 1,
  ...
  },
  {
    ...
    slug: "l2",
    nameJa: "L2 時間因果",
    descriptionJa: "原因・結果・時系列・プロセスの連鎖",
    colorClass: "layer-2",
  },
  {
    index: 3,
    slug: "l3",
    nameJa: "L3 主体心理",
    descriptionJa: "主観・感情・動機・意図・認知の層",
    colorClass: "layer-3",
  },
  {
    index: 4,
    slug: "l4",
    nameJa: "L4 社会評価",
    descriptionJa: "規範・価値判断・社会的意味・評判の層",
    colorClass: "layer-4",
  },
  {
    index: 5,
    slug: "l5",
    nameJa: "L5 制度形式",
    descriptionJa: "法・制度・手続き・形式的ルールの層",
    colorClass: "layer-5",
  },
];
...
async function main() {
  ...
  const existing = await prisma.packVersion.findUnique({
    where: { key: PACK_KEY },
  });
  if (existing && existing.version >= PACK_VERSION) {
    console.log(`Pack ${PACK_KEY} v${PACK_VERSION} already seeded. Skipping.`);
    return;
  }
  ...
  for (const layer of layers) {
    const record = await prisma.layerDefinition.upsert({
      ...
      update: {
        nameJa: layer.nameJa,
        descriptionJa: layer.descriptionJa,
  ...
  await prisma.mappingRule.createMany({
    data: [
      {
        fromLayerId: layerMap["l3"],
        toLayerId: layerMap["l4"],
        pattern: "感情",
        replacement: "社会的表現",
        condition: "score_l3 > 0.5",
        priority: 10,
      },
      {
        fromLayerId: layerMap["l4"],
        toLayerId: layerMap["l5"],
        pattern: "規範",
        replacement: "法的規定",
        condition: "score_l4 > 0.4",
        priority: 8,
      },
      {
        fromLayerId: layerMap["l0"],
        toLayerId: layerMap["l2"],
        pattern: "生成",
        replacement: "プロセス展開",
        priority: 5,
      },
      {
        fromLayerId: layerMap["l1"],
        toLayerId: layerMap["l3"],
        pattern: "選択",
        replacement: "意図的選好",
        priority: 6,
      },
  ...
  for (const concept of concepts) {
    ...
    const created = await prisma.concept.upsert({
      ...
      update: { titleJa: conceptData.titleJa, summary: conceptData.summary },
  ...
  await prisma.packVersion.upsert({
    where: { key: PACK_KEY },
    update: { version: PACK_VERSION },
    create: { key: PACK_KEY, version: PACK_VERSION },
  });
