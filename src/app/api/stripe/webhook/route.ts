...
export async function POST(req: NextRequest) {
  ...
  await prisma.billingEvent.upsert({
    ...
    create: { stripeEventId: event.id, type: event.type, payloadJson: event as unknown as Record<string, unknown> },
    update: {},
  });
    ...
    case "customer.subscription.updated": {
      ...
      await prisma.networkMembership.upsert({
        ...
        create: {
          ...
          status: sub.status,
          currentPeriodEnd: new Date((sub as Record<string, unknown>).current_period_end as number * 1000),
